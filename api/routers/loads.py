...
@router.get("", response_model=list[LoadSummaryOut])
...
@router.get("/{load_id}", response_model=LoadSummaryOut)
def get_load_endpoint(load_id: int) -> LoadSummaryOut:
    return LoadSummaryOut.from_domain(get_load(load_id))


@router.post("/{load_id}/transition", response_model=TransitionResultOut)
...
@router.post("/{load_id}/assign-driver", response_model=TransitionResultOut)
